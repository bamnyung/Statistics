...
err1_A_dvar_out = err1/100

###### A.power ######
power_A_dvar_out<-function(d,d_var){
  m<-5; n<-5;
...
d1<-power_A_dvar_out(1,d_var); d2<-power_A_dvar_out(2,d_var); d3<-power_A_dvar_out(3,d_var)
pow<-data.frame(d1=d1,d2=d2,d3=d3)
plot(c(1,2,3),pow[1,],xlab="delta",ylab="power",type="l",col="orange",
     main="case A\"':X~N & Y~N",ylim=c(0,1),lwd=5)
lines(c(1,2,3),pow[2,],type="l",col="red",lwd=0.5)
...
err1_B_dvar_out = err1_B/100

###### B.power ######
power_B_dvar_out<-function(d,d_var){
  m<-5; n<-5;out<-3
  mu_x<-0; mu_y<-d; var_x<-1; var_y<-d_var
...
d1<-power_B_dvar_out(1,d_var); d2<-power_B_dvar_out(2,d_var); d3<-power_B_dvar_out(3,d_var)
pow<-data.frame(d1=d1,d2=d2,d3=d3)
plot(c(1,2,3),pow[1,],xlab="delta",ylab="power",type="l",col="orange",
     main="case B\"':X~N & Y~Gamma",ylim=c(0,0.4),lwd=3)
lines(c(1,2,3),pow[2,],type="l",col="red",lwd=0.5)
lines(c(1,2,3),pow[3,],type="l",col="blue")
lines(c(1,2,3),pow[4,],type="l",col="green")
lines(c(1,2,3),pow[5,],type="l",col="black")
...
###### C.type1err ######
m<-5; n<-5; 
mu_x<-2; mu_y<-2; var_x<-1; var_y<-d_var
alpha_x<-4; beta_x<-1/2; alpha_y<-4/d_var; beta_y<-d_var/2
...
err1_C_dvar_out = err1_C/100

###### C.power ######
power_C_dvar_out<-function(d,d_var){
  m<-5; n<-5; out<-3
...
d1<-power_C_dvar_out(1,d_var); d2<-power_C_dvar_out(2,d_var); d3<-power_C_dvar_out(3,d_var)
pow<-data.frame(d1=d1,d2=d2,d3=d3)
plot(c(1,2,3),pow[1,],xlab="delta",ylab="power",type="l",col="orange"
